...
int main()
{
	...
	shell::GameShell dab("Dots&Boxes");
	...
	auto IndexLimitParamsCheck = [](const shell::ParamsList& list)->bool {
		if (list.size() != 1)
			return false;
	...
	//page state
	state->AddFunction("next", "set random edge in current state.", state_shell::NextState);
	...
	state->AddFunction("set", "set a edge in current state.", state_shell::SetEdge, IndexLimitParamsCheck);
	state->AddFunction("remove", "remove a edge in current state.", state_shell::RemoveEdge, IndexLimitParamsCheck);
	...
	state->AddFunction(
		"edge",
		"to create state with edges.",
		[](State& state, const shell::ParamsList& list)->bool {
		size_t num = atoi(list[0].c_str());
		if (num < 60 && num > 0)
		{
